...
Let's take a look at the summary, and then postHoc test our results.  We already know from our one-way ANOVA that fertilzer affects the yield, but the information about density will be new.  
```{r}
summary(two.way);
TukeyHSD(two.way)
```
In the summary, we reject the null hypothesis that the density does not affect the yield.  
In the postHoc test, we see that there's a significant difference between the two levels of planting density; fertilzer type 3-1; and fertilzer 3-2.  
...
#### **Two way plot**
Let's use a two-way plot to show this information.  We can split our output into different groups in a two-way plot.  It'll look like a scatterplot, but clustered in a laterial line shape, with two lines for each density group.  
